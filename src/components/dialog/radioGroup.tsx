  ...
  return (
    <RadioGroup defaultValue='audio-high' onValueChange={onValueChange}>
      ...
      <div className='flex items-center space-x-2'>
        ...
        <Label htmlFor='r1'>Alto {'(720p)'} {formats[0].fileLength !== null && formats[0].fileLength && <span className='text-sm text-muted-foreground'>{convertToMB(parseInt(formats[0].fileLength))}MB</span>}</Label>
      </div>
      <div className='flex items-center space-x-2'>
        <RadioGroupItem value='audio-low' id='r2' />
        <Label htmlFor='r2'>Estandar {'(480p)'} {formats[1].fileLength !== null && formats[1].fileLength && <span className='text-sm text-muted-foreground'>{convertToMB(parseInt(formats[1].fileLength))}MB</span>}</Label>
      </div>

      <h6 className='font-medium text-sm mt-3 mb-2'>Video</h6>
      <div className='flex items-center space-x-2'>
        <RadioGroupItem value='video-high' id='r3' />
        <Label htmlFor='r3'>Alto {'(720p)'} {formats[2].fileLength !== null && formats[2].fileLength && <span className='text-sm text-muted-foreground'>{convertToMB(parseInt(formats[2].fileLength))}MB</span>}</Label>
      </div>
      <div className='flex items-center space-x-2'>
